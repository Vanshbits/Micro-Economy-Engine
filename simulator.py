...
Gender = ["Male", "Female"]
energy = 100
money_range = [50, 200, #poverty class
         300, 800, #middle class
         1000, 2000 #upper class
         ]
wage = [
        30 #poverty_class
        , 60 #working_class
...
supplies_range = [10,40, #poverty class
                  40, 80, #middle class
                  80, 100 #upper_class
                  ]
...
class character:
    def __init__(self, Name, Gender, energy, money, state, x_cord, y_cord, supplies, wage, metabolism, char_class):
        self.Name = Name
        self.Gender = Gender
        self.energy = energy
        self.money = money
    ...
    def live(self):
        ...
        if self.supplies <= self.metabolism*self.distance(store_cord) + 5 and self.state != "sleep" and self.money != 0:
...
def character_generator(population):
    for i in range(population):
        ...
        characters[f"agent_{i}"] = character(f"agent_{i}",random.choice(Gender), energy,money, state,
                                              random.randint(residential_cord[0],residential_cord[1]),
                                                random.randint(residential_cord[0],residential_cord[1]),
                                                supplies, wages, random.choice(metabolism),char_class )
...
def run_sim():
    ...
    end_hr = 48
    dic = {"HOUR": [], "NAME": [],"ENERGY": [], "MONEY": [], "STATE": [], "X_CORD": [], "Y_CORD":[], "DISTANCE": [], "SUPPLIES": [], "DESTINATION": [], "CLASS": []}
